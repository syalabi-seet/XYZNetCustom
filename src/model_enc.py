...
from tensorflow.keras.layers import (
    Conv2D, MaxPool2D, LeakyReLU, Flatten, Dense, Dropout, Resizing, Rescaling, add)
...
class CompiledMAELoss(Loss):
    ...
    def call(self, y_true, y_pred):
        xyz_loss = self.xyz_mae(y_true, y_pred)
        return xyz_loss
        ...
        for i in range(self.local_depth):
            local_subnet.add(Conv2D(
                **self.conv_params,
                ...
                activation=self.activation if i != self.local_depth - 1 else "sigmoid",
                name=f"conv_{i}"))
